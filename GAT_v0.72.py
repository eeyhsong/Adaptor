...
Use conformer as the code base to reconstruction the Global Adaptive Transformer 

The same feature extractor

Significant improvement than v0.65!! 

Find the best results of all the epochs - bad use
"""
...
class SelfTrans_src(nn.Sequential):
    def __init__(self,
                 emb_size,
                 num_heads=10,
                 drop_p=0.5,
...
class DATrans():
    ...
    def get_data(self):  # get source and target data
        ...
        return self.source_data, self.source_label, self.full_data, self.full_label, self.test_data, self.test_label
    ...
    def train(self):
        ...
        sour_img, sour_label, img, label, test_data, test_label = self.get_data()
        ...
        for e in range(self.n_epochs):
            ...
            for i, (img, label, sour_img, sour_label) in enumerate(self.dataloader):
                ...
                if (i + 1) % 1 == 0:
                    ...
                    loss_U = loss_Joint_cls + loss_Joint_adv + self.lambda_cen/5 * loss_Cen  # + 0.5 * loss_Joint_irr + self.lambda_cen * loss_Cen
            ...
            print(
                'Epoch: %d  Loss_D: %.4f  Loss_G: %.4f  Loss_cls_sour: %.4f  Loss_cls_targ: %.4f  Loss_cen: %.4f  Train_acc: %.5f  Test_acc: %.5f'
                % (e, loss_D, loss_Joint_adv, loss_cls_sour, loss_cls_targ, loss_Cen, tacc, acc))
    ...
    def test(self, e):
        ...
        feature_extractor.load_state_dict(torch.load("./model/sub%d_Enc_72.pth" % self.nSub))
        classifier.load_state_dict(torch.load("./model/sub%d_Cls_72.pth" % self.nSub))
